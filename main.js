...
const table = new Tabulator("#room-list", {
    ...
    columns: [
        ...
        {
            title: "Players", field: "players", hozAlign: "center", formatter: printPlayers, sorter: function (a, b, aRow, bRow, column, dir, sorterParams) {
...
function printFlag(cell, formatterParams) {
    const flagCode = cell.getValue();
    return `<div data-hook="flag" class="flagico f-${flagCode}"></div>`;
}

function printPlayers(cell, formatterParams) {
    const data = cell.getData();
    return `${data.players}/${data.maxPlayers}`;
}

function printPassword(cell, formatterParams) {
    const data = cell.getData();
...
class Reader {
    ...
    decodeCharacter(a, b) {
        ...
        else if (252 == (c & 254)) d = a.getUint8(b + 1), e = a.getUint8(b + 2), f = a.getUint8(b + 3), g = a.getUint8(b + 4), k = a.getUint8(b + 5), c = (c & 1) << 30 | (d & 63) << 24 | (e & 63) << 18 | (f & 63) << 12 | (g & 63) << 6 | k & 63,
            b += 6;
...
function parseRooms(a) {
    const rooms = [];
    let totalPlayers = 0;
    while (0 !== a.data.byteLength - a.bytesOffset) {
        const room = {
            id: a.getID(),
